#!/usr/bin/env python3
"""Installable CLI bridge for Codex-facing HREVN workflows."""
...
def require_key(value: str | None) -> str:
    if value:
        return value
    raise SystemExit("HREVN_API_KEY is required. Run `hrevn-codex health-check` first if you only want connectivity.")
...
def build_parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(description="Call the HREVN managed API from Codex-facing workflows.")
    root.add_argument(
        "--base-url",
...
def run_baseline(base_url: str, api_key: str, path: str | Path) -> int:
...
def run_download_bundle(base_url: str, api_key: str, bundle_id: str, output: str | None) -> int:
    payload = get_bytes(base_url, api_key, f"/v1/bundles/{urllib.parse.quote(bundle_id)}/download")
    if output:
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "baseline":
            return run_baseline(base_url, require_key(args.api_key), DEFAULT_BASELINE_INPUT)
        if args.command == "baseline-check":
            return run_baseline(base_url, require_key(args.api_key), args.input)
